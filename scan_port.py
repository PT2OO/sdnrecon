...
def usage():
	...
	print("-p", "       Port to scan (all or <p1>to<pN> or <a specified port>)     No") 
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) < 1):
        ...
        if ("-p" in params):
            try:
               ...
               usage()
               exit()
            if ((params[params.index("-p")+1]).lower() == "all"):
                ports = []
                for i in range(1, 65535):
                    ports.append(int(i))
            elif ("to" in (params[params.index("-p")+1]).lower()):
                pa = (params[params.index("-p")+1]).split("to")
                ports = []
                for i in range(pa[0],pa[1]):
                    ports.append(int(i))
            elif int(params[params.index("-p")+1]) in range(1,65535):
                ports = []
                ports.append(int(params[params.index("-p")+1]))
            else:
                usage()
                exit()
        else:
    ...
    main(target, ports, advanced_udp)
