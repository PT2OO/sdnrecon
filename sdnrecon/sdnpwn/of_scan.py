...
def getPorts(port):
  ...
  if(port == None):
    ...
    ports_arr = [6633,6634,6653]
  elif("," in port):
    ports_arr = port.split(",")
  elif("-" in port):
    #ports_arr = []
...
def run(params):
  ...
  port_input = sdnpwn.getArg(["--port", "-p"], params)
  ...
  for host in range(startIndex, endIndex):
    ...
    for p in ports_x:
      try:
        versions = enumerateVersions(targetHost, p, sockTimeout)
      except:
        pass
      
      if(versions is not None):
          targetRes.append((p, versions))
    if(len(targetRes) > 0):
